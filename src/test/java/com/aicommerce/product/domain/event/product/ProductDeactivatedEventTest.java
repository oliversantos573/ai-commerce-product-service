...
class ProductDeactivatedEventTest {
    ...
    @Test
    void shouldCreateProductDeactivatedEventSuccessfully() {

        ProductId productId = new ProductId(UUID.randomUUID());
        Instant occurredAt = FIXED_INSTANT;

        ProductDeactivatedEvent event =
                new ProductDeactivatedEvent(productId, occurredAt);

        assertEquals(ProductEventType.PRODUCT_DEACTIVATED, event.getEventType());
